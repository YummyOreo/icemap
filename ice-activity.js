...
function renderSightings(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if ((sighting.type == "protest" || sighting.type == "poly") && activeFilter == "sightings") {
            continue;
        }
        if (!(sighting.type == "protest" || sighting.type == "poly") && activeFilter == "protests") {
            continue;
        }
    ...
    .then(data => {
        ...
        for (i in buttons) {
            ...
            button.addEventListener("click", (e) => {
                ...
                let capitalizeFirst = activeFilter[0].toUpperCase() + activeFilter.substring(1);
                activeButtonInner.innerText = capitalizeFirst;

                resetSightings()
                renderSightings(data)
                resetHeatmap()
                if (!heatmapButton.classList.contains("button-subtle") && activeFilter != "protests") {
                    renderHeatmap(data)
                }
            })
        }
